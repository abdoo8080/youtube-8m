...
def main(unused_argv):
  ...
  tf.compat.v1.train.import_meta_graph(meta_graph_location, clear_devices=True)

  input_tensor_name = tf.compat.v1.get_collection("input_batch_raw")[0].name
  num_frames_tensor_name = tf.compat.v1.get_collection("num_frames")[0].name

  # Create output graph.
  saver = tf.compat.v1.train.Saver()
  tf.compat.v1.reset_default_graph()

  input_feature_placeholder = tf.compat.v1.placeholder(
        tf.float32, shape=(None, None, 1152))
  num_frames_placeholder = tf.compat.v1.placeholder(tf.int32, shape=(None, 1))
